...
export const httpReq = (id: string, { headers, params, body, session, user, ips, ip, cookies, signedCookies }: Request) => formatObject({
  id,
  headers,
  params,
  body: { ...body },
...
export const httpRes = (id: string, start: number, data: any, enc: BufferEncoding, headers: Record<string,string>, status: number) => formatObject({
  id,
  headers: { ...headers },
  timeMs: msAgo(start),
  status,
